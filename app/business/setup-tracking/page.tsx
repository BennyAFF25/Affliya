...
export default function SetupTrackingPage() {
  ...
  const isOnboarding =
    (searchParams?.get('onboard') || '').toLowerCase() === '1' ||
    (searchParams?.get('onboard') || '').toLowerCase() === 'tracking';
  ...
  useEffect(() => {
    if (isOnboarding && testStatus === 'ok') {
      const t = setTimeout(() => {
        router.push('/business/my-business');
      }, 2000);
      return () => clearTimeout(t);
    }
  }, [isOnboarding, testStatus, router]);
                  ...
                  {offers.length === 0 ? (
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center gap-3 flex-wrap">
                        ...
                        {isOnboarding && testStatus === 'ok' && (
                          <button
                            type="button"
                            onClick={() => router.push('/business/my-business')}
                            className="ml-auto px-4 py-2 rounded-md bg-[#0f172a] hover:bg-[#1e293b] text-xs font-semibold text-[#7ff5fb] border border-[#1e293b]"
                          >
                            Finish setup &amp; return to Control Hub
                          </button>
                        )}
                      </div>
