...
export default function PostIdeasPage() {
  ...
  const [selectedPostInDropdown, setSelectedPostInDropdown] =
    useState<PostIdea | null>(null);
        ...
        if (error) {
          ...
          if (typedData.length > 0) {
            ...
            if (uniqueEmails.length > 0) {
              const { data: profileRows, error: profileError } = await (
                supabase as any
              )
              ...
              if (profileError) {
                console.error(
                  "[❌ Error fetching profile avatars]",
                  profileError,
                );
              } else if (profileRows) {
    ...
    <div className="post-ideas-theme min-h-screen bg-[var(--background)] px-4 py-8 text-[var(--foreground)] sm:px-8 lg:px-10">
      ...
      {posts.length === 0 ? (
        <p className="text-gray-600">
          New post ideas will appear here once affiliates submit them.
        </p>
      ) : (
        <>
              ...
              >
                ...
                <div className="flex flex-col sm:items-end gap-2 w-full sm:w-auto">
                  ...
                  <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                    <button
                      onClick={() =>
                        handleStatusChange(post.id, "approved", post)
                      }
                    ...
                    <button
                      onClick={() =>
                        handleStatusChange(post.id, "rejected", post)
                      }
      ...
      <section className="mt-12 text-center text-gray-500">
        {posts.length === 0 && (
          <p>
            No post ideas have been submitted yet. Once affiliates submit posts,
          ...
          return (
            <div className="fixed inset-0 z-50 flex items-center justify-center px-3 sm:px-4">
              ...
              <div className="relative z-50 w-full max-w-sm rounded-3xl bg-[#05080a] border border-[#151b1f] shadow-[0_20px_60px_rgba(0,0,0,0.75)] overflow-hidden">
                ...
                {kind === "email" &&
                  (() => {
                    const { subject, body } = parseEmailFromCaption(
                      selectedPost.caption,
                    );
                    return (
                      <div className="border-t border-[#151b1f] bg-[#05080a]">
                        <div className="px-4 pt-3 pb-2">
                          ...
                          <div className="text-[11px] text-white/50">
                            From: {selectedPost.affiliate_email} • To: (audience
                            segment)
                          </div>
                        </div>
                ...
                {kind === "forum" &&
                  (() => {
                    const { titleOrUrl, body } = parseForumFromCaption(
                      selectedPost.caption,
                    );
                    return (
                      <div className="border-t border-[#151b1f] bg-[#05080a]">
