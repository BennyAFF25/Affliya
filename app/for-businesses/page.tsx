    ...
    <div className="marketing-businesses-theme min-h-screen flex flex-col overflow-x-hidden bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-[#0b1a1b] via-[#0b0b0b] to-black text-white">
      ...
      <section className="relative mx-auto max-w-7xl px-6 pt-14 pb-10">
        ...
        <p className="mt-5 max-w-2xl text-gray-300 text-lg leading-relaxed">
          Partners fund media. You approve what runs. Ads go live only after
          your sign‑off — with one simple tag for clicks, carts and conversions.
        </p>
      ...
      <section className="mx-auto max-w-7xl px-6 pt-4 pb-2">
        <div className="flex flex-wrap gap-3 rounded-2xl border border-[#16393b] bg-[#0e1617] px-4 py-4">
          <span className="rounded-full border border-[#223d3e] bg-[#111b1c] px-4 py-1.5 text-sm text-gray-300">
            Keep brand control
          </span>
          <span className="rounded-full border border-[#223d3e] bg-[#111b1c] px-4 py-1.5 text-sm text-gray-300">
            Unified tracking
      ...
      <section className="mx-auto max-w-7xl px-6 py-10">
        ...
        <div className="grid md:grid-cols-4 gap-4">
          <StepCard
            step="1"
            title="Connect"
            copy="Link Meta & Stripe. Add a small site tag."
          />
          <StepCard
            step="2"
            title="Create an offer"
            copy="Set commission & rules."
          />
          <StepCard
            step="3"
            title="Approve"
            copy="Partners fund and submit. You sign‑off."
          />
          <StepCard
            step="4"
            title="Payouts"
            copy="Confirmed sales pay out automatically."
          />
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-6 pb-10">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <FAQ
            ...
            a="Partners do. They pre‑fund spend from their own wallet — you never prepay."
          />
          <FAQ
            q="Do partners get full account access?"
            a="No. They submit campaigns for approval and run under your guardrails."
          />
          <FAQ
            q="How do payouts work?"
            a="When a partner sale is confirmed, Stripe automatically displays commisions to settle on your dashboard."
          />
          <FAQ
            q="What do we need to start?"
            a="Connect Meta and Stripe during onboarding and add a lightweight tracking tag."
          />
        </div>
      </section>
    </div>
...
function FAQ({ q, a }: { q: string; a: string }) {
  return (
    <details className="group relative overflow-hidden rounded-2xl border border-[#16393b] bg-[#050809] text-left transition-all duration-300 hover:border-[#00C2CB]/70 hover:shadow-[0_0_25px_rgba(0,194,203,0.18)]">
      <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3 md:px-5 md:py-4">
        ...
        <span className="flex h-7 w-7 items-center justify-center rounded-full border border-[#1f3a3c] bg-[#0b1516] text-[#7ff5fb] text-base leading-none transition-transform duration-200 group-open:rotate-45">
          +
        </span>
      </summary>
